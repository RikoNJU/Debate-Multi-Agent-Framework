...
目标是把 `paper-review-backend/dev` 中已经验证过的论文解析、历史建议检索、辅助检测和报告能力，以适配器方式迁入当前 LangGraph 多智能体后端，并逐步替换 Demo 服务。系统首先服务南京大学人工智能学院毕业论文评审，同时通过院校、院系、论文类型和评分规则配置，为后续推广保留边界。
...
- PDF 上传、MinerU 解析、Markdown 结构化和论文类型/章节阶段识别。
- 原 Step 3 历史专家建议 RAG。
- 原 Step 6 总结建议和 Step 7 综合评分实现。
...
- 迁入论文类型、章节阶段分类规则和 PromptService。
- 为 Prompt、模型和分类规则增加版本字段。
- 编写旧 Chroma 数据审计和重建脚本，生成稳定 chunk ID。
- 元数据仅保留院系、论文类型、章节阶段、建议类别、匿名案例 ID 和版本。
...
本分支只实现两个基础能力：M1 历史建议 RAG，以及 M2 的 MinerU 云解析子阶段。MinerU 范围包括 PDF 校验、上传、异步轮询、安全解压、产物契约和后端接口；Markdown 章节解析器留到下一分支。旧 Step 6/7、评分校准和报告等能力均不在本分支实现。
