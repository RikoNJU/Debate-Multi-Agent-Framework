...
- Step 4 的旧章节语义字段仍由新 Chair 适配生成，尚未逐项迁移旧 Step 4 Prompt。
...
- 已以 Markdown 识别语义章节，并用 MinerU `content_list.json` 输出稳定块、chunk、页码、坐标、图表和公式位置。
...
### M4：Step 5/6/7 生产适配器（核心规则已完成）

- 已迁移三类 Step 5 标准、Step 6 建议选择规则、Step 7 十二项定义与总分公式。
...
后端链路和 M3 的 Step 1/2 分类适配，并完成 Step 5/6/7 核心规则适配。范围包括
PDF 校验、MinerU 上传与轮询、安全解压、Markdown 章节解析、论文类型自动分类、三套
章节阶段分类、`content_list.json` 版面结构化、一体化任务接口、旧历史建议 Chroma
兼容、三类工作量标准、可追溯建议汇总、Demo 数据隔离、块级证据引用校验和任务失败
语义。脱敏真实论文的 MinerU 格式验证、旧库审计重建、历史评分 RAG、外部学术证据、
